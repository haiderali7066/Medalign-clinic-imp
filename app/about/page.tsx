...
export default function About() {
  const physiotherapists = [
    { name: "Anum Zafar", designation: "Consultant Physiotherapist", qualifications: ["DPT", "MS-NMPT"] },
    { name: "Rohma Samman", designation: "Senior Physiotherapist", qualifications: ["DPT", "MS-NMPT"] },
    { name: "Hamza Tahir", designation: "Senior Physiotherapist", qualifications: ["DPT", "MS-MSKPT"] },
    { name: "Narimane Afzal", designation: "Senior Physiotherapist", qualifications: ["DPT", "MS-CPPT"] },
    { name: "Dur-e-Shahwar", designation: "Senior Physiotherapist", qualifications: ["DPT", "MS-MSKPT"] },
    { name: "Abeera tul Jannat Satti", designation: "Senior Physiotherapist", qualifications: ["DPT", "MS-MNPT"] },
    { name: "Afifa Manzoor", designation: "Clinical Physiotherapist", qualifications: ["DPT"] },
  ];

  const values = [
    { icon: Award, title: 'Excellence', description: 'We maintain the highest standards in clinical practice and patient care.' },
    { icon: Users, title: 'Compassion', description: 'Every patient receives personalized attention and empathetic support.' },
    { icon: Zap, title: 'Innovation', description: 'We use cutting-edge techniques and technology for optimal outcomes.' },
  ];
    ...
    <div className="bg-white text-foreground overflow-x-hidden box-border">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-muted">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="max-w-4xl mx-auto">
          <motion.h2 variants={itemVariants} className="text-4xl font-bold mb-8">Our Story</motion.h2>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="text-center mb-16">
            <motion.h2 variants={itemVariants} className="text-4xl font-bold mb-4">Core Values</motion.h2>
            <motion.p variants={itemVariants} className="text-lg text-muted-foreground">What drives us every day</motion.p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {values.map((value, idx) => {
              ...
              return (
                ...
                >
                  <motion.div whileHover={{ y: -5 }} className="bg-muted p-8 border border-border hover:shadow-xl transition-shadow rounded-lg">
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-muted">
        <div className="max-w-7xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="text-center mb-16">
            <motion.h2 variants={itemVariants} className="text-4xl font-bold mb-4">Meet Our Team</motion.h2>
            <motion.p variants={itemVariants} className="text-lg text-muted-foreground">Experienced professionals dedicated to your care</motion.p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {physiotherapists.map((member, idx) => (
              ...
              >
                <motion.div whileHover={{ scale: 1.03 }} className="bg-white border border-border p-6 rounded-lg shadow-sm hover:shadow-lg transition-shadow flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-primary text-white flex items-center justify-center rounded-full mb-4 text-xl font-bold">Dr</div>
                  <h3 className="text-xl font-bold mb-2">{member.name}</h3>
                  <p className="text-primary font-semibold mb-2">{member.designation}</p>
                  <p className="text-muted-foreground text-sm">{member.qualifications.join(", ")}</p>
                </motion.div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-primary text-white">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <motion.div key={idx} variants={itemVariants}>
                <motion.div whileHover={{ scale: 1.05 }} className="text-5xl font-bold mb-2">{stat.value}</motion.div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative h-96 w-full max-w-full">
            <Image src="/6.jpeg" alt="Clinic facility" fill className="object-cover border border-border rounded-lg" />
          </motion.div>
        </div>
      </section>
